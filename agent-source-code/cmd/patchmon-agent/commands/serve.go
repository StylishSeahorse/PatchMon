	...
	*backoff = time.Second
	...
	for {
		...
		case "pty_open":
			...
			dispatchWSMessage(out, wsMsg{kind: "pty_open", ptyData: payload.SessionID, ptyUsername: payload.Username, ptyTerminal: payload.Terminal, ptyCols: payload.Cols, ptyRows: payload.Rows})
		case "pty_input":
			if !validBastionSessionID(payload.SessionID) || len(payload.Data) > 256*1024 {
				continue
			}
			dispatchWSMessage(out, wsMsg{kind: "pty_input", ptyData: payload.SessionID, sshProxyData: payload.Data, ptySequence: payload.Sequence})
		case "pty_resize":
			if !validBastionSessionID(payload.SessionID) {
				continue
			}
			dispatchWSMessage(out, wsMsg{kind: "pty_resize", ptyData: payload.SessionID, ptyCols: payload.Cols, ptyRows: payload.Rows})
		case "pty_signal":
			if !validBastionSessionID(payload.SessionID) {
				continue
			}
			dispatchWSMessage(out, wsMsg{kind: "pty_signal", ptyData: payload.SessionID, ptySignal: payload.Signal})
		case "pty_close":
			if validBastionSessionID(payload.SessionID) {
				dispatchWSMessage(out, wsMsg{kind: "pty_close", ptyData: payload.SessionID})
			}
		case "ssh_tunnel_open":
			if !cfgManager.IsIntegrationEnabled("ssh-bastion-enabled") {
			...
			dispatchWSMessage(out, wsMsg{kind: "ssh_tunnel_open", ptyData: payload.SessionID})
		case "ssh_tunnel_input":
			if !validBastionSessionID(payload.SessionID) || len(payload.Data) > 512*1024 {
				continue
			}
			dispatchWSMessage(out, wsMsg{kind: "ssh_tunnel_input", ptyData: payload.SessionID, sshProxyData: payload.Data})
		case "ssh_tunnel_close":
			if validBastionSessionID(payload.SessionID) {
				dispatchWSMessage(out, wsMsg{kind: "ssh_tunnel_close", ptyData: payload.SessionID})
			}
		case "ssh_proxy":
...
func runPatch(patchRunID, patchType string, packageNames []string, dryRun, rebootIfRequired bool) error {
	...
	runSyspatch := func(args ...string) (error, bool) {
		...
		sink.Flush()
		stepOutput, err := runStreamingPatchStep(ctx, sink, env, "syspatch", args...)
		lastStepOutput = stepOutput
		if err == nil {
			return nil, false
		}
...
func runPatchBrew(ctx context.Context, httpClient *client.Client, patchRunID, patchType string, packageNames []string, dryRun bool) error {
	...
	runStep := func(isDryRunStep bool, errTag, errFmt, name string, args ...string) (error, bool) {
		...
		sink.WriteString(formatCmd(name, args...))
		sink.Flush()
		_, err := runStreamingPatchStep(ctx, sink, brewEnv, execName, execArgs...)
		if err == nil {
			return nil, false
		}
...
func runPatchDarwin(ctx context.Context, httpClient *client.Client, patchRunID, patchType string, packageNames []string, dryRun bool) error {
	...
	runStep := func(isDryRunStep bool, errTag, errFmt, name string, args ...string) (error, bool) {
		sink.WriteString(formatCmd(name, args...))
		sink.Flush()
		_, err := runStreamingPatchStep(ctx, sink, os.Environ(), name, args...)
		if err == nil {
			return nil, false
		}
		if isDryRunStep && isDryRunExit1Success(err, fullOutput.String()) {
